...
constexpr moveList *generatePawnMoves(const Board &board, moveList *mList,
    ...
    if (Type == M_CAPTURE || Type == CHECK_EVASION || Type == CAP_QUIET) {
        ...
        Bitboard eastAttacks =
              (eAttack == NORTH + EAST ? shift<NORTH + EAST>(singlePushPawns)
                                       : shift<SOUTH + EAST>(singlePushPawns)) &
              capture;
        Bitboard westAttacks =
              (wAttack == NORTH + WEST ? shift<NORTH + WEST>(singlePushPawns)
                                       : shift<SOUTH + WEST>(singlePushPawns)) &
              capture;
...
constexpr moveList *generateLegal(const Board &board, moveList *mList) {
    ...
    Bitboard pinners =
          (board.pieces(QUEEN, ~C) | board.pieces(ROOK, ~C)) & orth;
    ...
    if (__builtin_popcountll(board.bCheckPcs()) == 1 &&
        !(board.checkPcs & board.pieces(PAWN, ~C))) {
    ...
    if ((T == M_QUIET || T == CAP_QUIET) && board.canCastle(C)) {
        ...
        if (cr & KING_SIDE && !inCheck) {
            ...
            if (!(betweenMask & board.pieces())) {
                ...
                if (!attacked) {
                    temp.addMove(
                          encodeMove(kingSq, Square(kingSq + 2), K_CASTLE),
                          false, false);
                }
            }
        }
        ...
        if (cr & QUEEN_SIDE && !inCheck) {
            ...
            if (!((7ULL << (kingSq - 3)) & board.pieces())) {
                ...
                if (!attacked) {
                    temp.addMove(
                          encodeMove(kingSq, Square(kingSq - 2), Q_CASTLE),
                          false, false);
                }
            }
        }
