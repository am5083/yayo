...
int TracePeek::calculate(std::tuple<int, int, int> info) {
    ...
    for (int i = 0, w = 0; w < 487; i += 2, w++) {
        if (trace[i] - trace[i + 1]) {
            const int eval =
                  (trace[i] - trace[i + 1]) * ((mgPhase * MgScore(weights[w]) +
                                                egPhase * EgScore(weights[w])) /
                                               24);
...
void TracePeek::print() {
    ...
    for (int i = 0; i < 8; i++) {
        std::cout << t.passedPawn[i][WHITE] - t.passedPawn[i][BLACK]
                  << std::endl;
    }
    ...
    for (int i = 0; i < 8; i++) {
        std::cout << t.doubledPawns[i][WHITE] - t.doubledPawns[i][BLACK]
                  << std::endl;
    }
    ...
    for (int i = 0; i < 8; i++) {
        std::cout << t.isolatedPawns[i][WHITE] - t.isolatedPawns[i][BLACK]
                  << std::endl;
    }
    ...
    for (int i = 0; i < 8; i++) {
        std::cout << t.backwardPawns[i][WHITE] - t.backwardPawns[i][BLACK]
                  << std::endl;
    }
    ...
    for (int i = 0; i < 9; i++) {
        std::cout << t.knightMobility[i][WHITE] - t.knightMobility[i][BLACK]
                  << std::endl;
    }
    ...
    for (int i = 0; i < 14; i++) {
        std::cout << t.bishopMobility[i][WHITE] - t.bishopMobility[i][BLACK]
                  << std::endl;
    }
    ...
    for (int i = 0; i < 15; i++) {
        std::cout << t.rookMobility[i][WHITE] - t.rookMobility[i][BLACK]
                  << std::endl;
    }
    ...
    for (int i = 0; i < 28; i++) {
        std::cout << t.queenMobility[i][WHITE] - t.queenMobility[i][BLACK]
                  << std::endl;
    }
